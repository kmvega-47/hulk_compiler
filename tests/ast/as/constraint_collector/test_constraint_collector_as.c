...
void test_collector_as_single_param(void)
{
    printf("\n\n------------ Inicio test: test_collector_as_single_param ------------\n");
    dm_global = dm_create();
    global_tables_init();

    printf("type A {} f(x) => x as A  -->  x debe conformar a A\n");
    ...
    ASTNode *body = (ASTNode *)ast_as_create(x_ref, "A", 1, 17);
    ...
    printf("Tipo inferido para x: %s (esperado: A)\n", x_type ? x_type->name : "NULL");
    CU_ASSERT_PTR_EQUAL(x_type, (TypeDescriptor *)a_type);
...
void test_collector_as_undefined_type(void)
{
    printf("\n\n------------ Inicio test: test_collector_as_undefined_type ------------\n");
    dm_global = dm_create();
    global_tables_init();

    printf("f(x) => x as NoExiste  -->  error, x queda Object\n");

    List *params = list_create(0, NULL, (free_func)symbol_binding_destroy, NULL, NULL);
    list_append(params, symbol_binding_create("x", NULL, NULL, 1, 5));
    ASTNode *x_ref = (ASTNode *)ast_variable_reference_create("x", 1, 15);
    ASTNode *body = (ASTNode *)ast_as_create(x_ref, "NoExiste", 1, 17);
    FunctionDefinitionNode *f = ast_function_definition_create("f", params, NULL, body, 1, 1);
...
void test_collector_as_complex_target(void)
{
    printf("\n\n------------ Inicio test: test_collector_as_complex_target ------------\n");
    dm_global = dm_create();
    global_tables_init();

    printf("f(x) => (x + 1) as Number  -->  x Number\n");

    List *params = list_create(0, NULL, (free_func)symbol_binding_destroy, NULL, NULL);
    list_append(params, symbol_binding_create("x", NULL, NULL, 1, 5));
    ASTNode *x_ref = (ASTNode *)ast_variable_reference_create("x", 1, 16);
    ASTNode *add = (ASTNode *)ast_binary_create(OP_ADD, x_ref, (ASTNode *)ast_number_literal_create(1.0, 1, 20), 1, 18);
    ASTNode *body = (ASTNode *)ast_as_create(add, "Number", 1, 23);
    FunctionDefinitionNode *f = ast_function_definition_create("f", params, NULL, body, 1, 1);
...
int main(void)
{
    printf("\n=== Iniciando tests de ConstraintCollector (as) ===\n");

    if (CUE_SUCCESS != CU_initialize_registry())
        return CU_get_error();

    CU_pSuite suite = CU_add_suite("ConstraintCollector As Tests", NULL, NULL);
    if (!suite)
    {
        CU_cleanup_registry();
        return CU_get_error();
    ...
    CU_add_test(suite, "test_collector_as_single_param", test_collector_as_single_param);
    CU_add_test(suite, "test_collector_as_undefined_type", test_collector_as_undefined_type);
    CU_add_test(suite, "test_collector_as_complex_target", test_collector_as_complex_target);
