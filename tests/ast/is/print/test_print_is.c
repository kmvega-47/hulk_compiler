...
void test_print_is(void)
{
    printf("\n\n------------ Inicio test: test_print_is ------------\n");
    global_tables_init();
...
void test_print_is_complex_target(void)
{
    printf("\n\n------------ Inicio test: test_print_is_complex_target ------------\n");
    global_tables_init();

    printf("Caso: (a + b) is Number (linea 5, columna 9)\n");
...
int main(void)
{
    printf("\n=== Iniciando tests de IsNode (print) ===\n");

    if (CUE_SUCCESS != CU_initialize_registry())
        return CU_get_error();

    CU_pSuite suite = CU_add_suite("IsNode Print Tests", NULL, NULL);
    if (!suite)
    {
        CU_cleanup_registry();
        return CU_get_error();
    ...
    CU_add_test(suite, "test_print_is", test_print_is);
    CU_add_test(suite, "test_print_is_complex_target", test_print_is_complex_target);
